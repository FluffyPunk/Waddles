...
const { getStreamInfo } = require("./Trovo");
...
async function wsWork(raw, client) {
  ...
  if (type !== "CHAT" || !data || data?.chats[0].type !== 5012) return;
  ...
  if (streamers.includes(username)) {
    ...
    if (content === "stream_on") {
      ...
      setTimeout(async () => {
        ...
        const streamInfo = await getStreamInfo(streamDB.nickname);
        streamDB.discordList.forEach((channelId) => {
          embed.send(client, channelId, streamInfo);
        });
